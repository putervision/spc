/**
 * checker.js - Core logic for @putervision/spc, a space-proofing code analysis tool.
 * Analyzes JavaScript, Python, and C/C++ codebases for patterns that compromise performance,
 ...
 * what triggers the check. Patterns are grouped by language (JavaScript, Python, C/C++).
 */
const LANGUAGE_PATTERNS = {
  javascript: {
    extensions: ['.js'], // File extensions for JavaScript files
    patterns: {
 ...
 *
 * @param {string[]} lines - Array of code lines to analyze.
 * @param {string} language - Language of the code ("javascript", "python", "c").
...
function checkReturnUsage(lines, language) {
  ...
  const ignoreList = {
    javascript: ['console.log', 'console.error', 'setTimeout', 'setInterval'],
    python: ['print', 'sys.exit', 'logging.info'],
    c: ['printf', 'fprintf', 'exit'],
  };

  // Additional check for security-critical functions whose returns must be handled
  const criticalFunctions = {
    javascript: ['fetch', 'crypto.', 'http.get'],
    python: ['requests.get', 'urllib.request.urlopen', 'os.system'],
    c: ['system', 'recv', 'send'],
  };
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const isIgnored = (ignoreList[language] || []).some((func) =>
      line.includes(func)
    );
    if (isIgnored) {
      continue;
    ...
    const hasVoidIndicators =
      line.includes('void') || // C void functions
      line.includes('console.') || // JS console methods
      line.includes('print'); // Python print
    ...
    if (!hasVoidIndicators) {
      ...
      const isCritical = (criticalFunctions[language] || []).some((func) =>
        line.includes(func)
      );
      if (isCritical) {
        issues.push(
 ...
 * Analyzes JavaScript, Python, and C/C++ codebases to identify performance, reliability, and
