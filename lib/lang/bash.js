const BashPatterns = {
  ...
  patterns: {
    ...
    // Pipe from curl/wget into shell execution
    curl_pipe_bash:
      /\b(curl|wget)\s+[^|;]+?\s*\|\s*(?:sudo\s+)?(?:bash|sh|zsh|source)\b/g,

    // Unattended or non-interactive sudo / NOPASSWD
    sudo_nopasswd: /\bsudo\s+-[nS]\b|\bNOPASSWD:\s*ALL\b/g,

    // Non-cryptographic pseudo-random variable
    insecure_random: /\$RANDOM\b/g,

    // Direct shell parameter expansion into database queries
    sql_injection: /\b(mysql|psql|sqlite3)\s+-[^e]*e\s+['"][^'"]*\$/g,
  },
