const GoPatterns = {
  ...
  patterns: {
    ...
    weak_crypto: /\b(md5\.New|sha1\.New)\b/g,

    // Example: rand.Intn(10)
    insecure_random: /\b(math\/rand|rand\.(?:Int|Float|Read))\b/g,
    ...
    // SQL Injection
    sql_injection:
      /\b(?:db\.Query|db\.Exec|db\.QueryRow)\s*\(\s*(?:fmt\.Sprintf|[^,)]*\+)/g,

    // Format String
    format_string: /\bfmt\.(?:Printf|Sprintf|Fprintf)\s*\(\s*[^"%][^,)]+\s*\)/g,

    // SSRF
    ssrf_risk:
      /\bhttp\.(?:Get|Post)\s*\(\s*(?:r\.URL\.Query|r\.FormValue|fmt\.Sprintf)/g,
