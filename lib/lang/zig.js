const ZigPatterns = {
  ...
  patterns: {
    ...
    exposed_secrets:
      /(?:const|var)\s+\b(api_key|apikey|secret|password|passwd|auth_token|access_token|private_key)\b\s*:\s*\[\]const\s+u8\s*=\s*["'][^"']{4,}["']/gi,

    // Insecure PRNG
    insecure_random: /\bstd\.rand\.DefaultPrng\b/g,

    // Format string vulnerability
    format_string: /\bstd\.fmt\.allocPrint\s*\([^,]+,\s*[a-zA-Z_]\w*\s*,/g,

    // Use after free
    use_after_free:
      /\ballocator\.free\s*\(\s*(\w+)\s*\)\s*;[\s\S]{0,100}?\b\1\b/g,
  },
