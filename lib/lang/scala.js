const ScalaPatterns = {
  ...
  patterns: {
    ...
    insecure_random: /\b(scala\.util\.Random|java\.util\.Random)\b/g,
    unsafe_file_op:
      /\b(scala\.io\.Source\.fromFile|Files\.write)\s*\([^,]*[^&]*\)/g,
    insufficient_logging: /\b(println\s*\([^;]*\))/g,
    unsanitized_exec: /\b(scala\.sys\.process\s*\.\s*!\s*\([^)]*\${[^}]*\))/g,
    exposed_secrets:
      /\b(val|var)\s+\b(api_key|apikey|secret|password|passwd|auth_token|access_token|private_key)\b\s*=\s*['"][^'"]{4,}['"]/gi,
  },
