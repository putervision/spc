const RustPatterns = {
  ...
  patterns: {
    ...
    multiple_returns:
      /fn\s+\w+\s*\([^)]*\)\s*(?:->\s*\w+)?\s*\{[\s\S]*?return[\s\S]*?return/g,
    ...
    weak_crypto: /\b(md5::compute|sha1::Sha1::new)\b/g,

    // Insecure PRNG in security context
    insecure_random: /\brand::(?:random|thread_rng)\b/g,
    ...
    exposed_secrets:
      /(?:const|static)\s+\b(API_KEY|SECRET|PASSWORD|AUTH_TOKEN|PRIVATE_KEY|api_key|secret|password|token)\b\s*:\s*&str\s*=\s*["'][^"']{4,}["']/gi,

    // SQL Injection in raw format strings
    sql_injection: /\b(?:sqlx::query|client\.query)\s*!\s*\(\s*&?format!/g,

    // Race condition / memory unsafety in multithreading
    race_condition: /\b(static\s+mut|std::sync::atomic::Ordering::Relaxed)\b/g,
