...
async function scanCodebase(
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (createSums) {
        checksums.push(`${fileHash}  ${filePath}`);
      } else if (
        checkSumIndex &&
        checkSumIndex[filePath] &&
        checkSumIndex[filePath] !== fileHash
      ) {
        results.push({
          file: filePath,
          language: '',
          issues: [`Check sum mismatch for file`],
        });
      }
...
async function loadChecksums(checksumFilePath) {
  try {
    ...
    throw new Error(`Failed to load checksums: ${error.message}`);
  }
}
