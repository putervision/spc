...
const DEFAULT_MAX_FILE_SIZE = 5 * 1024 * 1024; // 5 MB max file size
...
async function safeRealPath(p) {
  if (typeof fs.realpath === 'function') {
    try {
      ...
      if (e && (e.code === 'EACCES' || e.message === 'Permission denied')) {
        throw e;
      }
      return p;
    }
...
/**
 * Precomputes character start index for every line.
 * @param {string} content - Full file text.
 * @returns {number[]} Array of character start offsets.
 */
...
/**
 * Fast binary search to find 1-indexed line number for character index.
 * @param {number[]} offsets - Precomputed line start offsets.
 * @param {number} charIndex - Match index.
...
/**
 * Ensures regular expression contains the global (/g) flag for matchAll.
 * @param {RegExp} pattern - Regex to normalize.
 * @returns {RegExp}
 */
  ...
  if (langConfig.patterns) {
    for (const [issueType, pattern] of Object.entries(langConfig.patterns)) {
      ...
      for (const match of matches) {
        const lineNum = getLineNumber(lineOffsets, match.index);
        rawIssues.push({
          message: `${match[0]}`,
          issueType,
          lineNum,
 ...
 * @param {Object} options - Additional scan options (category, aiOnly, skipAi, configPath, pluginsDir, maxFileSize).
...
async function scanCodebase(
  ...
  try {
    ...
    const maxFileSize =
      config.maxFileSize || options.maxFileSize || DEFAULT_MAX_FILE_SIZE;
      ...
      await Promise.all(
        batch.map(async (file) => {
          ...
          try {
            ...
            // Prevent OOM by skipping oversized files
            ...
            // Only hash if generating checksums or verifying existing manifest
            const shouldHash =
              createSums || (checkSumIndex && checkSumIndex[file]);
            if (shouldHash) {
