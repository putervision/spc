...
async function scanCodebase(
  ...
  try {
    const resolvedDir = await fs.realpath(directory);
    const spcIgnoreRules = await loadSpcIgnore(resolvedDir);
    const combinedIgnore = [...ignorePatterns, ...spcIgnoreRules];
      ...
      await Promise.all(
        batch.map(async (file) => {
          ...
          try {
            ...
            if (!realPath.startsWith(resolvedDir)) return;

            if (filePath.match(CHECKSUMS_FILE)) return;

            if (combinedIgnore?.length) {
              const isIgnored = combinedIgnore.some((ignore) => {
                if (ignore.includes('*')) {
...
/**
 * Create SHA-256 hash using stream-based reading for low memory usage.
 */
