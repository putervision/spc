...
const LANGUAGE_PATTERNS = {
  ...
  java: {
    ...
    patterns: {
      ...
      // Flags unvalidated inputs, vulnerable to RF-injected malicious data
      unsafe_input:
        /\b(request\.getParameter|System\.in|BufferedReader\.readLine|Scanner\.next)\s*\(/g,
  ...
  go: {
    ...
    patterns: {
      ...
      unsafe_input:
        /\b(r\.URL\.Query|r\.FormValue|os\.Args|bufio\.NewReader\s*\(\s*os\.Stdin\s*\)\.ReadLine)/g,
  ...
  rust: {
    ...
    patterns: {
      ...
      unsafe_input:
        /\b(req\.uri\(\)\.query|std::env::args|std::io::stdin\(\)\.read_line)/g,

      // Example: reqwest::get("http://api")
      network_call:
        /\b(reqwest::get|std::net::TcpStream::connect|http::Client)/g,
...
module.exports = { scanCodebase, PATTERN_INFO };
